...
bool IsMulticastIP(u_long ulIP);
// ulIP	in network byte order
// multicast IP: E0.x.x.x ~ EF.x.x.x

bool IsPublicIP(u_long ulIP); 
// Private IP:	192.168.xxx.xxx/10.xxx.xxx.xxx/172.[16(0x10)-31(0x1f)].xxx.xxx
//				127.0.0.1  0.0.0.0, 169.254/16 (auto-configured)
// All others: public
...
public:
	CUdpSocket();
	virtual	~CUdpSocket();
	virtual	bool Init(PCSTR cpszListenPort, bool bBroadcast=false, PCSTR cpszDottedIpOfInterface=NULL) = 0;	
	// cpszListenPort: NULL for auto-assignment
	// bBroadcast: to Tx and Rx broadcast packets for not, default is false;
	// cpszDottedIpOfInterface: specify interface IP to bind to, such as "192.168.1.10". NULL to bind to INADDR_ANY
	virtual	bool Init(unsigned short ushListenPort, bool bBroadcast=false, PCSTR cpszDottedIpOfInterface=NULL) = 0; 
	...
	// dwTimeoutsMs: 0 for blocking call (INFINITE), 1 (SOCKET_IO_NO_WAIT) for nonblocking calls; 
...
public:
	...
	// dwTimeoutsMs: 0 for blocking calls and 1 (SOCKET_IO_NO_WAIT) for nonblocking calls
	// Note: If Send() returns false, you should call IsBroken() to
	//			see if Send() failed due to connection broken or timeout.
	virtual bool RawReceive(DWORD& dwLen, LPBYTE pData, DWORD dwTimeoutsMs=30*1000) = 0;
	// dwTimeoutsMs: 0 for blocking calls and 1 (SOCKET_IO_NO_WAIT) for nonblocking calls

	virtual bool IsReadable(DWORD dwTimeoutMs) = 0;//in millisecond time out
...
public:
	...
	virtual bool Init(LPCSTR cpszServicePort, int qLen=5, bool bReuseAddr=false, PCSTR cpszDottedIpOfInterface=NULL) = 0;
	// pszTxProtocol: "tcp" or "udp"
	// Note: the support of "udp" has been moved to UdpSocket.
	// bReuseAddr: SOL_SOCKET, SO_REUSEADD and/or SO_REUSEPORT for stream-based socket, must call before bind
	// cpszDottedIpOfInterface: specify interface IP to bind to, such as "192.168.1.10". NULL to bind to INADDR_ANY
	virtual bool Init(unsigned short ushServicePort, int qLen=5, bool bReuseAddr=false, PCSTR cpszDottedIpOfInterface=NULL) = 0;
	// Note: the support of "udp" has been moved to UdpSocket.
	// bReuseAddr: SOL_SOCKET, SO_REUSEADD and/or SO_REUSEPORT for stream-based socket, must call before bind
	// cpszDottedIpOfInterface: specify interface IP to bind to, such as "192.168.1.10". NULL to bind to INADDR_ANY
